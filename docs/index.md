...
Read-only and panic-free by construction: `#![forbid(unsafe_code)]`, no `unwrap`/`expect`/unchecked indexing in production parsers, every length/offset bounds-checked, and `cargo fuzz` targets (`open`/`read`/`inspect`/`forensic`) that must never panic on crafted bytes. Decoding is **differentially validated against `qemu-img convert`** and a real CirrOS cloud image; `inspect()` is validated against real qemu-img-produced backing-file, snapshot, and LUKS images. See [Validation](validation.md) and [Implementation Notes](implementation-notes.md).

See the project [README](https://github.com/SecurityRonin/qcow2-forensic) for the full related-crate map.
